...
def test_product_suite_card_layout_in_browser(desktop_page, path):
    ...
    for i in range(count):
        ...
        assert dist_to_bottom < 80.0, f"En {path}, acción {i} no está anclada al fondo (dist={dist_to_bottom}px)"
    ...
    for row_key in h3_tops_by_row:
        ...
        if len(h3_tops) > 1:
            ...
            assert diff_h3 < 8.0, f"En {path}, los títulos h3 en fila {row_key} no inician a la misma altura (diff={diff_h3}px)"
            
            diff_p = max(p_tops) - min(p_tops)
            assert diff_p < 8.0, f"En {path}, los párrafos p en fila {row_key} no inician a la misma altura (diff={diff_p}px)"
            
            max_diff = max(bottoms) - min(bottoms)
            assert max_diff < 8.0, f"En {path}, las acciones en la fila {row_key} no tienen la misma base (diff={max_diff}px)"
